...
test('stageBridgeReleaseBundle extracts a verified zip and activateBridgeRelease writes current pointer', async () => {
  ...
  try {
    ...
    await activateBridgeRelease({ version: release.version, releasesDir, stagedDir })

    assert.equal(await readFile(path.join(releasesDir, release.version, 'dist/index.js'), 'utf8'), 'console.log("updated")\n')
    assert.deepEqual(JSON.parse(await readFile(path.join(releasesDir, 'current.json'), 'utf8')).releasePath, release.version)
  } finally {
    await rm(releasesDir, { recursive: true, force: true })
  }
})

function signedRelease(input: { bytes: Buffer; privateKey: ReturnType<typeof generateKeyPairSync>['privateKey'] }): BridgeRelease {
