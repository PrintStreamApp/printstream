...
export function verifyBridgeReleaseBundle(input: {
  ...
  verifyDetachedSha256Signature({
    sha256: input.release.bundle.sha256,
    signature: input.release.bundle.signature,
    publicKeyPem: input.publicKeyPem,
    artifactName: 'Bridge release bundle'
  })
}
...
export async function stageBridgeReleaseBundle(input: {
  ...
  const stagingDir = path.join(input.releasesDir, '.staging', input.release.sourceFingerprint.slice(0, 12))
...
export async function activateBridgeRelease(input: {
  /** Directory name under the releases dir; the build's short fingerprint. */
