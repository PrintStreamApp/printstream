...
const THREE_MF_PARSER_CACHE_VERSION = 7
...
export async function readBridgeLibraryThreeMfIndex(filePath: string): Promise<BridgeLibraryThreeMfIndex> {
  ...
  try {
    modelSettingsPlates = parseModelSettingsPlates((await readEntry(filePath, 'Metadata/model_settings.config')).toString('utf8'))
  } catch {
    modelSettingsPlates = []
  }
...
function parseModelSettingsPlates(xml: string): ModelSettingsPlateMetadata[] {
  ...
  for (const block of plateBlocks) {
    ...
    for (const match of block.matchAll(/<model_instance\b[^>]*>[\s\S]*?<metadata\s+key="object_id"\s+value="(\d+)"\s*\/>[\s\S]*?<\/model_instance>/g)) {
      ...
      if (!Number.isInteger(objectId) || objectId <= 0 || seenObjectIds.has(objectId)) continue
