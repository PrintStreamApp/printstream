...
`apps/bridge/src/library-3mf.ts` is a hand-kept **mirror** of `apps/api/src/lib/three-mf-reader.ts`
(the read/index half of the API's `three-mf-*` modules). Because files are bridge-owned, this bridge
parser is what normally produces the 3MF index the web sees (via the `library.inspect3mf` RPC); the
API's copy only runs for the local-copy/fallback parse and for slice-time 3MF rewriting.

When changing the parsed 3MF **index shape** (e.g. a per-plate field like `objects`):
1. Apply the change in **both** parsers (`library-3mf.ts` here and `apps/api/src/lib/three-mf-reader.ts`).
