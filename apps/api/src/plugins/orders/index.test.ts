...
test('orders routes create templates, expand order prints, and complete the order across confirm and manual completion', async () => {
  ...
  const plugin = createOrdersPlugin({
    ...
    inspectBridgeLibraryThreeMf: async () => ({ plates: [], projectFilaments: [], compatiblePrinterModels: [], supportFilamentIds: [], printerProfileName: null, processProfileName: null, geometryOnly: false, objectExport: false, needsSettingsRepair: false, settingsRepairReasons: [], projectVersion: null, slicedWithFilamentTrackSwitch: false }),
