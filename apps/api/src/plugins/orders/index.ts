    ...
    async register(context) {
      ...
      context.router.post('/templates', requireRequestPermission(PRINTS_DISPATCH_PERMISSION), async (request, response) => {
        ...
        const variants = await Promise.all(
          parsed.data.variants.map((variant, position) => resolveTemplateVariant(context.prisma, services, variant, position))
        )
...
async function resolveTemplateVariant(
  ...
  const items = await Promise.all(
    variant.items.map((item, itemPosition) => resolveTemplateItem(prismaClient, deps, item, itemPosition))
  )
...
async function resolveTemplateItem(
  ...
  if (isThreeMf) {
    const index = await readTemplateItemIndex(file, deps).catch(() => null)
    if (!index?.plates.some((plate) => plate.index === item.plate)) {
      throw badRequest(`Plate ${item.plate} does not exist in ${file.name}`)
    }
  }
