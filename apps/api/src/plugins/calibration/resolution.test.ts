...
test('a spool-specific result beats any identity match', () => {
  ...
  const picked = resolveCalibrationValue(candidates, polymakerSilver, target)
...
test('a non-matching identity field disqualifies the result', () => {
  ...
  assert.equal(resolveCalibrationValue(candidates, polymakerSilver, target), null)
})
...
test('identity match applies across printers of the same model (no spool needed)', () => {
  ...
  assert.equal(resolveCalibrationValue(candidates, freshRoll, target)?.value, 0.022)
})
