...
test('settings write accepts a quick-start-only update when auth is disabled', async () => {
  prisma.setting.upsert = ((async () => ({})) as unknown) as typeof prisma.setting.upsert
  prisma.setting.findUnique = ((async () => null) as unknown) as typeof prisma.setting.findUnique
  ...
  await withSettingsApp({
    ...
    const response = await fetch(`${baseUrl}/api/settings`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ quickStartDismissed: true })
    })

    assert.equal(response.status, 200)
    assert.equal((await response.json() as { quickStartDismissed: boolean }).quickStartDismissed, true)
  })
})
