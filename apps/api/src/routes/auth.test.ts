...
const originalDefaultTenantSlug = env.DEFAULT_TENANT_SLUG
...
afterEach(() => {
  env.DEFAULT_TENANT_SLUG = originalDefaultTenantSlug
...
test('auth bootstrap stays in platform context even when DEFAULT_TENANT_SLUG is configured', async () => {
  env.DEFAULT_TENANT_SLUG = 'local'
  // An enabled provider keeps the install out of wide-open mode, so the only
  // mechanism that could scope this request would be the legacy env fallback.
  authProviderRegistry.register({
    id: 'auth-local',
    label: 'Local Auth',
    enabled: true,
