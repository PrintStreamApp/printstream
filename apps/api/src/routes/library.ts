    ...
    handler(request, response, (error: unknown) => {
      if (error instanceof MulterError) {
        if (error.code === 'LIMIT_FILE_SIZE') {
          ...
          next(new HttpError(413, `File exceeds ${limitMb} MB upload limit`))
          return
        }
...
libraryRouter.post('/folders', requireRequestPermission(LIBRARY_MANAGE_PERMISSION), async (request, response) => {
  ...
  try {
    ...
    if (isUniqueViolation(error)) throw conflict('A folder with that name already exists here')
    throw error
  }
})
...
/**
 * Raw STL bytes for a library file, scoped to viewers (not downloaders) and
 * without an audit-log entry, so the web client can render a model preview
 * thumbnail for STL files (which carry no embedded image). 3MF/gcode keep using
 * `/thumbnail`; STL has no server-side renderer, so the bytes are shipped for
 * client-side rendering by the model-previewer plugin.
 */
libraryRouter.get('/:id/mesh', requireRequestPermission(LIBRARY_VIEW_PERMISSION), async (request, response) => {
  const fileId = requireRouteParam(request.params.id, 'File id')
  const row = await prisma.libraryFile.findUnique({ where: { id: fileId } }) as LibraryFileRow | null
  if (!row) throw notFound('File not found')
  if (row.kind !== 'stl') throw notFound('No mesh available')
...
libraryRouter.post('/:id/reprint', requireRequestPermission(PRINTS_DISPATCH_PERMISSION), async (request, response) => {
  ...
  if (parsed.data.amsMapping && parsed.data.amsMapping.length > 0) {
    printPayload.ams_mapping = parsed.data.amsMapping
  }
...
async function sendLibraryFileThumbnail(
  ...
  const signal = requestAbortSignal(request, response)
  const plateParam = Number(request.query.plate)
  const plateIndex = Number.isFinite(plateParam) && plateParam > 0 ? Math.floor(plateParam) : 1
