...
function uploadImportFile(field: string) {
  ...
  return (request: Parameters<typeof handler>[0], response: Parameters<typeof handler>[1], next: Parameters<typeof handler>[2]) => {
    handler(request, response, (error: unknown) => {
      if (error instanceof MulterError && error.code === 'LIMIT_FILE_SIZE') {
        next(new HttpError(413, `Imported file exceeds ${Math.floor(MAX_IMPORT_UPLOAD_BYTES / (1024 * 1024))} MB limit`))
        return
      }
...
editorRouter.post(
  ...
  async (request, response) => {
    ...
    const { baseFileId, baseVersionId, mode, sceneEdit } = parsed.data
    ...
    try {
      ...
      annotateRequestAuditLog(request, {
        ...
        metadata: { fileId: created.id, mode, baseFileId: baseFileId ?? null, importCount: imports.length }
      })
...
editorRouter.post('/scaffold/:id/discard', requireRequestPermission(LIBRARY_UPLOAD_PERMISSION), async (request, response) => {
  ...
  const row = await prisma.libraryFile.findFirst({ where: { id: fileId, tenantId }, select: { id: true } })
