...
export async function pruneHiddenLibraryFiles(
  ...
  for (const row of stale) {
    ...
    await deps.deleteLibraryFileBytes(row).catch(() => undefined)
...
export async function pruneRecycledLibraryFiles(
  ...
  for (const row of stale) {
    ...
    for (const version of row.versions) {
      await deps.deleteLibraryFileBytes(version).catch(() => undefined)
    }
    removed += 1
  }
  if (removed > 0) {
...
export async function runArtifactMaintenance(): Promise<void> {
  ...
  const removedCoverArtifacts = coverCache.removedCoverFiles + coverCache.removedAliasFiles
  ...
  if (
    ...
    && coverCache.removedAliasFiles === 0
