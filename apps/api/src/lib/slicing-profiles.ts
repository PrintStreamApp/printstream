...
export function buildBuiltinSlicingProfileId(kind: SlicingProfileKind, name: string): string {
...
function extractProfileMetadata(record: Record<string, unknown>): Partial<SlicingProfileSummary> {
  // Keep BambuStudio JSON fields as the source of truth for filtering. UI
  // name parsing is only a fallback for project/imported profiles missing these fields.
  return omitEmptyMetadata({
