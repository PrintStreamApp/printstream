/**
 * API-side printer storage access for bridge-owned printers.
 *
 * Thin wrapper over the bridge's `storage.*` RPCs: upload/download, directory
 * listing, rename/delete, file size, and ZIP-entry reads all run on the bridge
 * that owns the printer (the real FTPS lives in `@printstream/bridge-runtime`).
 * Direct base64 RPC uploads are capped at `MAX_RPC_UPLOAD_BYTES`; larger files
 * must go through bridge library storage, and downloads stream in bounded chunks.
 */
...
export async function readPrinterZipEntriesViaBridge(
	printer: Printer,
	remotePath: string,
	entryPaths: string[],
