...
import { deleteWorkspaceArtifactBytes, type WorkspaceArtifactDeps } from './workspace-artifacts.js'

function makeDeps(overrides: Partial<WorkspaceArtifactDeps> = {}): {
  deps: WorkspaceArtifactDeps
  ...
  const logs: string[] = []
  const deps: WorkspaceArtifactDeps = {
...
test('deletes bytes for every library file and version, both print-job artifacts, and detaches printers', async () => {
  ...
  const result = await deleteWorkspaceArtifactBytes('workspace-1', ctx.deps)
...
test('does nothing (no throw) for a workspace with no artifacts', async () => {
  const ctx = makeDeps()
  const result = await deleteWorkspaceArtifactBytes('empty', ctx.deps)
