...
const envSchema = z.object({
  ...
  DEFAULT_WORKSPACE_SLUG: z.string().default('workspace'),
  ...
  PRINTSTREAM_BRIDGE_BUILD_REVISION: optionalStringEnv(),
   ...
   * path. Leave unset for LAN-only installs; in that case Discord/ntfy
   * notifications will skip the snapshot embed.
   */
   ...
   * Example: when set to `printstream.app`, requests for
   * `acme.printstream.app` resolve to the `acme` tenant.
   */
  ...
  /**
   * Optional demo-data tenant slug hint. Runtime request routing no longer
   * falls back to a tenant automatically for anonymous requests.
   */
  DEFAULT_TENANT_SLUG: z.preprocess((value) => {
    if (typeof value !== 'string') return value
    const trimmed = value.trim().toLowerCase()
    return trimmed.length === 0 ? undefined : trimmed
  }, z.string().min(1).optional()),
...
export const env = {
  ...parsedEnv,
  PRINTSTREAM_BRIDGE_BUILD_REVISION: parsedEnv.PRINTSTREAM_BRIDGE_BUILD_REVISION ?? bridgeBuildMetadata.buildRevision,
