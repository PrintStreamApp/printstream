...
export function getActiveRecordedPrintJobId(printerId: string): string | null {
  return activeJobs.get(printerId) ?? null
}
...
async function resolveObservedLibraryFile(input: {
  ...
  const tenantId = await readPrinterTenantId(input.printerId).catch(() => null)
