...
import { printDispatcher, getRemotePrintTarget, prunePlateAmsMapping, sanitizeRemoteName } from './print-dispatcher.js'
...
test('assertBridgeAllowsPrinting rejects dispatch through a bridge that needs updating', async () => {
  for (const status of ['updateRequired', 'imageUpdateRequired', 'runnerUpdateRequired', 'unsupported']) {
...
test('assertBridgeAllowsPrinting allows compatible or unevaluated bridges', async () => {
  for (const updateStatus of ['current', 'updateAvailable', 'updateRecommended', null, undefined]) {
