...
const crashEvents: Array<{ bridgeId: string; recentCrashCount: number; workspaceId: string | null }> = []
const onCrashed = (event: { bridgeId: string; bridgeName: string; workspaceId: string | null; recentCrashCount: number }) => {
  crashEvents.push({ bridgeId: event.bridgeId, recentCrashCount: event.recentCrashCount, workspaceId: event.workspaceId })
}
...
function stubBridge(row: { name: string; workspaceId: string | null; lastCrashNotifiedAt: Date | null }): { updateArgs: unknown } {
...
test('records the crash summary, logs it for the workspace, and notifies on the first crash', async () => {
  const captured = stubBridge({ name: 'Store', workspaceId: 'workspace-a', lastCrashNotifiedAt: null })
  await ingestBridgeCrashReport({ bridgeId: 'bridge-a', sessionWorkspaceId: 'workspace-a', report: report() })
  ...
  const logged = getLogs(50, { workspaceId: 'workspace-a' }).find((entry) => entry.message.includes('Store'))
  assert.ok(logged, 'a workspace-scoped log entry was written')
  assert.equal(logged?.level, 'error')

  assert.deepEqual(crashEvents, [{ bridgeId: 'bridge-a', recentCrashCount: 1, workspaceId: 'workspace-a' }])
})
...
test('rate-limits the notification within the cooldown but still logs the crash', async () => {
  ...
  const captured = stubBridge({ name: 'Store', workspaceId: 'workspace-b', lastCrashNotifiedAt: justNotified })
  await ingestBridgeCrashReport({ bridgeId: 'bridge-b', sessionWorkspaceId: 'workspace-b', report: report({ recentCrashCount: 5 }) })
  ...
  const logged = getLogs(50, { workspaceId: 'workspace-b' }).find((entry) => entry.message.includes('crash-looping'))
  assert.ok(logged, 'a looping crash is still logged')
  assert.equal(crashEvents.length, 0, 'no notification within cooldown')
})

test('an unpaired bridge (no workspace) is recorded but not notified', async () => {
  const captured = stubBridge({ name: 'Lonely', workspaceId: null, lastCrashNotifiedAt: null })
  await ingestBridgeCrashReport({ bridgeId: 'bridge-c', sessionWorkspaceId: null, report: report() })

  assert.ok((captured.updateArgs as { data: Record<string, unknown> }).data.lastCrashAt instanceof Date)
  assert.equal(crashEvents.length, 0, 'no workspace to notify')
})
...
test('an unknown bridge id is ignored without throwing or emitting', async () => {
  ...
  await assert.doesNotReject(() => ingestBridgeCrashReport({ bridgeId: 'ghost', sessionWorkspaceId: 'workspace-a', report: report() }))
