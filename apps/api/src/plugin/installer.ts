...
const RESERVED_NAMES = new Set(['notifications', 'model-previewer'])
...
/** Stream-read helper for tests/debug; not used by the install path. */
export async function readManifestFromArchive(archivePath: string): Promise<PluginManifest> {
  const { manifest } = await readZip(archivePath)
  return manifestSchema.parse(JSON.parse(manifest.toString('utf8')))
}
