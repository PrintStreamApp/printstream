...
import { KNOWN_BAMBU_PRINTER_MODEL_KEYS, bambuModelKeysAreCompatible, canonicalBambuModelKey, normalizeBambuStudioPrinterModelOption, resolveBambuPrinterModelAliases } from '../lib/bambuPrinterModels'
import { COMMON_FILAMENT_COLOR_SWATCHES, commonFilamentColorName, filamentBackground, filamentTextColor, hasLoadedFilament, resolveFilamentColorSwatches, resolveFilamentDisplay, resolveProjectFilamentColorName, type FilamentColorSwatchOption } from '../lib/filamentColor'
import { resolveLoadedMaterialPreset } from '../lib/loadedMaterialProfileSelection'
...
/**
 * Mirror of the previewer's supported modes: STL, plated 3MF projects, and
 * plate-scoped sliced gcode 3MFs all render read-only from version bytes.
 */
function isVersionPreviewable(version: LibraryFileVersion): boolean {
  return version.kind === 'stl' || version.kind === '3mf' || version.name.toLowerCase().endsWith('.gcode.3mf')
}
...
function formatLibraryBytes(sizeBytes: number): string {
...
export function LibraryView() {
  ...
  const readDraggedItem = (event: DragEvent<HTMLElement>): LibraryDragItem | null => {
    ...
    try {
      ...
      if (parsed.type === 'file') {
        ...
        const file = visibleFiles.find((entry) => entry.id === parsed.id)
      ...
      if (parsed.type === 'files') {
        ...
        const draggedFiles = visibleFiles.filter((entry) => parsed.ids?.includes(entry.id))
      ...
      if (!parsed.id) return null
      const folder = allFolders.find((entry) => entry.id === parsed.id)
  ...
  const deleteFiles = async (files: LibraryFile[]) => {
  ...
  const deleteSelectedFiles = async () => {
    ...
    if (!confirmed) return
    await deleteFiles(selectedVisibleFiles)
    setSelectionMode(false)
  }
            ...
            >
              {canManageLibrary && !bridgeRootMode && (
                <Tooltip title="Recycle bin" variant="soft">
                  <IconButton size="sm" variant="plain" color="neutral" aria-label="Recycle bin" onClick={() => setRecycleBinOpen(true)}>
...
export function SliceFileModal({
  ...
  const slicingProfilesQuery = useQuery({
    ...
    queryFn: async ({ signal }) => {
      ...
      // The slicer can momentarily return an empty profile list while it is restarting or still
      // initializing, even though capabilities already report healthy. This query only runs once
      // the slicer is configured, so an empty result is never legitimate (BambuStudio always ships
      // builtin presets). Caching it as a success would strand the editor with only the project's
      // embedded profiles: no selectable machine profile to auto-pick, so `printerProfileId` stays
      // empty and the Slice button is silently disabled with no error. Treat empty as a transient
      // failure so React Query retries with backoff and surfaces a retryable error if it persists.
      if ((result.profiles?.length ?? 0) === 0) {
  ...
  const waitingForSlicingProfiles = shouldLoadSlicingProfiles && slicingProfilesQuery.isLoading && profiles.length === 0
  ...
  const missingFilamentProfile = projectFilaments.some((filament) => !filamentMaterialOptionIds[filament.projectFilamentId])
  const missingFilamentToolhead = materialToolheadOptions.length > 0 && projectFilaments.some((filament) => !filamentToolheadIds[filament.projectFilamentId])
  ...
  const desiredFilaments = useMemo<SceneEditFilament[] | null>(() => {
    if (!filamentCountChanged && !filamentColorsChanged) return null
  ...
  }, [filamentCountChanged, filamentColorsChanged, projectFilaments, addedFilamentIds, addedFilamentSourceIndex, baseProjectFilaments, materialOptions, filamentMaterialOptionIds, filamentColors])
  ...
  }), [removedFilamentIds, addedFilaments, addedFilamentSourceIndex, filamentColors, filamentMaterialOptionIds, filamentToolheadIds, filamentMaterialTypeFilters])
  ...
  const buildSubmitInput = (options?: { outputFileName?: string; outputFolderId?: string | null }): SliceFileSubmitInput => ({
    ...
    target: targetMode === 'realPrinter'
      ? {
          ...
          filamentMappings: buildFilamentMappings(projectFilaments, filamentMaterialOptionIds, filamentColors, filamentToolheadIds, materialOptions)
        }
      : {
          mode: 'manualProfile',
          printerProfileId,
  ...
  const selectedPrinterNozzleCount = selectedPrinterStatus?.nozzles.length ?? null
  ...
  const sliceController: SliceSettingsController = {
    ...
    filamentToolheadIds, setFilamentToolheadIds, filamentColors, setFilamentColors,
    setPrinterMaterialPickerFilamentId, handleMaterialOptionChange,
    desiredFilaments, onAddFilament: handleAddFilament, onRemoveFilament: handleRemoveFilament,
    materialsSnapshot, restoreMaterials
  }
                ...
                {group.options.map((option) => (
                  ...
                  >
                    <Stack direction="row" spacing={1} alignItems="center" sx={{ minWidth: 0, width: '100%' }}>
                      <Box sx={{ width: 24, height: 24, borderRadius: '50%', background: filamentBackground(option.colors, option.color, 'var(--joy-palette-neutral-800)'), border: '1px solid', borderColor: 'divider', flexShrink: 0 }} />
                      <Stack spacing={0} sx={{ minWidth: 0, alignItems: 'flex-start' }}>
                        ...
                        <Typography level="body-xs" textColor="text.tertiary" noWrap sx={{ minWidth: 0 }}>
                          {[option.metadata, formatPrinterMaterialNozzleLabel(option.nozzleId, 'short', selectedPrinterNozzleCount)].filter(Boolean).join(' · ')}
                        </Typography>
                      </Stack>
                    </Stack>
                  </Button>
 ...
 * toolpaths (via the previewer plugin's `library.overlays` slot, which works on the
...
export function SliceResultModal({
  ...
  return (
    ...
    {/* Sliced-toolpath preview of the (possibly still hidden) output file; the previewer
...
function sortSlicingProfiles(profiles: SlicingProfileSummary[]): SlicingProfileSummary[] {
...
function buildProjectSlicingProfiles(bakedIndex: ThreeMfIndex | null, kind: SlicingProfileSummary['kind']): SlicingProfileSummary[] {
...
function buildProjectSlicingProfileList(kind: SlicingProfileSummary['kind'], names: string[]): SlicingProfileSummary[] {
...
function mergeProjectSlicingProfiles(installedProfiles: SlicingProfileSummary[], projectProfiles: SlicingProfileSummary[]): SlicingProfileSummary[] {
...
function buildProjectSlicingProfileId(kind: SlicingProfileSummary['kind'], name: string): string {
  return `${PROJECT_SLICING_PROFILE_ID_PREFIX}${kind}:${encodeURIComponent(name)}`
}

function isProjectSlicingProfile(profile: SlicingProfileSummary): boolean {
  return profile.id.startsWith(PROJECT_SLICING_PROFILE_ID_PREFIX)
}

function isVisibleProcessProfile(profile: SlicingProfileSummary): boolean {
  const normalizedName = normalizedProfileText(profile.name)
  return !normalizedName.startsWith('fdm process')
}

function isVisibleFilamentProfile(profile: SlicingProfileSummary): boolean {
...
function ColorSwatchPicker({
  ...
  return (
    <Box>
      <Typography level="body-xs" textColor="text.tertiary" sx={{ mb: 0.5 }}>{title}</Typography>
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(28px, 1fr))', gap: 0.75 }}>
        <Box
          component="button"
          type="button"
          ...
          return (
            <Box
              ...
              sx={{
                ...
                border: (theme) => isSelected ? `2px solid ${theme.vars.palette.primary.solidBg}` : `1px solid ${theme.vars.palette.divider}`,
                boxShadow: isSelected ? '0 0 0 2px rgba(255,255,255,0.15) inset' : 'none',
                transition: 'transform 80ms ease',
                '&:hover': { transform: 'scale(1.08)' },
                '&:focus-visible': {
                  outline: (theme) => `2px solid ${theme.vars.palette.focusVisible}`,
...
function pickMachineProfileForPrinter(profiles: SlicingProfileSummary[], printer?: Printer | null): SlicingProfileSummary | null {
  if (!printer) return null
  return pickMachineProfileByName(profiles, printer.name, printer.model)
}

function pickMachineProfileByName(profiles: SlicingProfileSummary[], name: string | null | undefined, model: string): SlicingProfileSummary | null {
...
function isMachineProfileCompatible(profile: SlicingProfileSummary, model: string, nozzleDiameters: number[]): boolean {
...
function isProcessProfileCompatible(
...
function isFilamentProfileCompatible(
  profile: SlicingProfileSummary,
  selectedMachineProfile: SlicingProfileSummary | null,
  selectedProcessProfile: SlicingProfileSummary | null,
  model: string,
...
function matchesPrinterModel(profile: SlicingProfileSummary, model: string): boolean {
...
const LEGACY_MACHINE_SWITCH_MODELS = new Set(['H2D', 'H2DPRO', 'H2C'])

function buildLegacyMachineSwitchWarning(input: {
...
function resolveSliceDialogSourcePrinterModel(bakedIndex: ThreeMfIndex | null, fallbackModels: readonly string[]): string | null {
...
function resolveSliceDialogTargetPrinterModel(selectedPrinterModel: string, selectedMachineProfile: SlicingProfileSummary | null): string | null {
  return normalizeSliceDialogPrinterModel(selectedPrinterModel)
    ?? normalizeSliceDialogPrinterModel(selectedMachineProfile?.name)
}

function normalizeSliceDialogPrinterModel(value: unknown): string | null {
  return canonicalBambuModelKey(value)
}

function formatSliceDialogPrinterModel(value: string): string {
  switch (value) {
...
function matchesCompatiblePrinters(profile: SlicingProfileSummary, selectedMachineProfile: SlicingProfileSummary | null, model: string): boolean {
  ...
  const selectedModelKey = canonicalBambuModelKey(model) ?? canonicalBambuModelKey(selectedMachineProfile.name)
...
function matchesCompatiblePrints(profile: SlicingProfileSummary, selectedProcessProfile: SlicingProfileSummary | null): boolean {
...
function matchesNozzleDiameters(profile: SlicingProfileSummary, nozzleDiameters: number[]): boolean {
...
function matchesProfilePrinterTarget(profile: SlicingProfileSummary, selectedMachineProfile: SlicingProfileSummary | null, model: string): boolean {
...
function matchesProfileNozzleTarget(profile: SlicingProfileSummary, selectedMachineProfile: SlicingProfileSummary | null, nozzleDiameters: number[]): boolean {
...
function extractNozzleDiameterTokens(value: string): string[] {
  const tokens: string[] = []
...
function nozzleDiameterToken(value: number): string {
  return Number(value).toFixed(3)
}

function matchesPlateType(profile: SlicingProfileSummary, plateType: string): boolean {
 ...
 */
const BAMBU_STUDIO_PLATE_TYPES = ['cool_plate', 'engineering_plate', 'high_temp_plate', 'textured_pei_plate', 'supertack_plate']

function resolveCompatiblePlateTypes(file: LibraryFile, bakedIndex: ThreeMfIndex | null, selectedMachineProfile: SlicingProfileSummary | null, processProfiles: SlicingProfileSummary[]): string[] {
 ...
 */
function dedupePlateTypesByLabel(values: string[]): string[] {
...
function sortPlateTypesByBambuStudioOrder(values: string[]): string[] {
  const collator = new Intl.Collator(undefined, { numeric: true, sensitivity: 'base' })
...
function bambuStudioPlateTypeRank(value: string): number {
...
function resolveInitialPlateType(file: LibraryFile, bakedIndex: ThreeMfIndex | null): string {
  return bakedIndex?.plates.find((plate) => plate.plateType)?.plateType ?? file.plateTypeChips[0] ?? 'textured_pei_plate'
}

function resolveInitialNozzleDiameter(file: LibraryFile, printer: Printer | null | undefined, selectedMachineProfile: SlicingProfileSummary | null, bakedIndex: ThreeMfIndex | null): string {
...
function resolveSliceDialogNozzleDiameterOptions(file: LibraryFile, printer: Printer | null, machineProfiles: SlicingProfileSummary[], bakedIndex: ThreeMfIndex | null): string[] {
...
function resolveMachineProfileNozzleDiameters(profile: SlicingProfileSummary): number[] {
...
function buildSliceDialogToolheads(nozzleDiameter: string, nozzleFlow: PrinterNozzleFlow, status: PrinterStatus | undefined, model: string) {
  const diameter = Number.parseFloat(nozzleDiameter)
...
function buildSliceToolheadId(nozzleId: number): string {
...
function printerModelTextCandidates(model: string): string[] {
  if (model === 'unknown') return []
  return Array.from(new Set(resolveBambuPrinterModelAliases(model).flatMap(profileTextCandidates)))
}

function printerModelCompatibleTextCandidates(model: string): string[] {
...
function selectedPrinterCompatibilityTargets(selectedMachineProfile: SlicingProfileSummary | null, model: string): string[] {
...
function profileTargetMatchesSelectedPrinter(target: string, selectedTargets: string[]): boolean {
...
function extractProfilePrinterTargets(profile: SlicingProfileSummary): string[] {
...
function hasProfileToken(value: string, token: string): boolean {
...
function extractQuotedCompatibilityTargets(value: string | null | undefined): string[] {
  if (!value) return []
  return Array.from(value.matchAll(/["']([^"']+)["']/g), (match) => match[1]?.trim() ?? '').filter(Boolean)
}

function looksLikeGenericPrinterTarget(value: string): boolean {
  const normalized = normalizedProfileText(value)
  return normalized === 'default' || normalized === 'default printer' || normalized === 'any printer'
}

function buildInitialFilamentProfileSelection(file: LibraryFile, bakedIndex: ThreeMfIndex | null, profiles: SlicingProfileSummary[], machineProfile: SlicingProfileSummary | null = null): Record<number, string> {
...
function buildInitialFilamentMaterialOptionSelection(file: LibraryFile, bakedIndex: ThreeMfIndex | null, profiles: SlicingProfileSummary[], machineProfile: SlicingProfileSummary | null = null): Record<number, string> {
...
function buildBakedFilamentProfileSelection(bakedIndex: ThreeMfIndex, profiles: SlicingProfileSummary[], machineProfile: SlicingProfileSummary | null = null): Record<number, string> {
  const selections: Record<number, string> = {}
  for (const filament of bakedIndex.projectFilaments) {
...
function buildInitialFilamentColorSelection(file: LibraryFile, bakedIndex: ThreeMfIndex | null): Record<number, string> {
  const selections: Record<number, string> = {}
...
function buildInitialFilamentToolheadSelection(file: LibraryFile, bakedIndex: ThreeMfIndex | null): Record<number, string> {
  const selections: Record<number, string> = {}
  if (bakedIndex?.projectFilaments.length) {
    for (const filament of bakedIndex.projectFilaments) {
...
function buildSliceDialogProjectFilaments(
  file: LibraryFile,
...
function buildFilamentMappings(
...
function buildSliceMaterialOptions(profiles: SlicingProfileSummary[], loadedMaterials: SliceMaterialOption[]): SliceMaterialOption[] {
...
function dedupeSliceMaterialProfileOptions(options: SliceMaterialOption[]): SliceMaterialOption[] {
...
function formatPrinterMaterialSourceGroup(label: string, nozzleId: number | null, nozzleCount: number | null): string {
  const nozzleLabel = formatPrinterMaterialNozzleLabel(nozzleId, 'long', nozzleCount)
  return [label, nozzleLabel].filter(Boolean).join(' · ')
}

function formatPrinterMaterialNozzleLabel(nozzleId: number | null, variant: 'short' | 'long', nozzleCount: number | null): string | null {
  if (nozzleCount == null || nozzleCount <= 1) return null
  return formatNozzleLabel(nozzleId, variant, nozzleCount)
}

function groupSliceMaterialOptionsByGroup(options: SliceMaterialOption[]): Array<{ label: string; options: SliceMaterialOption[] }> {
...
function resolveMaterialTypeOptions(options: SliceMaterialOption[]): string[] {
  return sortMaterialTypesByBambuStudioOrder(Array.from(new Set(options.map((option) => option.materialType).filter(Boolean))))
}

function sortMaterialTypesByBambuStudioOrder(values: string[]): string[] {
  const collator = new Intl.Collator(undefined, { numeric: true, sensitivity: 'base' })
  return [...values].sort((left, right) => {
...
function bambuStudioMaterialTypeRank(value: string): number {
...
function narrowMaterialOptions(options: SliceMaterialOption[], materialType: string, keepId?: string | null): SliceMaterialOption[] {
...
function resolveProfileMaterialType(profile: SlicingProfileSummary): string {
  return normalizeMaterialTypeLabel(profile.filamentType) ?? extractMaterialType(profile.name)
}

function resolveLoadedMaterialType(filamentType: string | null | undefined, profile: SlicingProfileSummary | null | undefined, fallbackLabel: string): string {
...
function extractMaterialType(value: string): string {
...
function normalizeMaterialTypeLabel(value: string | null | undefined): string | null {
  const trimmed = value?.trim()
  if (!trimmed || normalizedProfileText(trimmed) === 'other') return null
  return trimmed
}

const BAMBU_STUDIO_MATERIAL_TYPE_ORDER = [
...
const BAMBU_STUDIO_MATERIAL_TYPE_MATCH_ORDER = [...BAMBU_STUDIO_MATERIAL_TYPE_ORDER].sort((left, right) => right.length - left.length)

function escapeRegExp(value: string): string {
  return value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
}

function resolveProfileMaterialBrand(profile: SlicingProfileSummary): string {
  return profile.filamentVendor?.trim() || extractMaterialBrand(profile.name)
}

function extractMaterialBrand(value: string): string {
...
function buildProfileMaterialOptionId(profileId: string): string {
  return `profile:${profileId}`
}

function hasLoadedMaterialDetails(trayName: string | null, filamentType: string | null, color: string | null): boolean {
  return Boolean(trayName?.trim() || filamentType?.trim() || color?.trim())
}

function normalizeSliceFilamentColor(value: string | null | undefined): string {
  const trimmed = value?.trim() ?? ''
...
function ensureOptionValues(values: string[], fallbacks: string[]): string[] {
  return Array.from(new Set([...values.filter(Boolean), ...fallbacks]))
}

function resolveInitialManualPrinterModel(file: LibraryFile, machineProfiles: SlicingProfileSummary[], fallback?: Printer['model']): string {
...
function ensurePrinterModelOptions(values: readonly string[], fallback?: string, machineProfiles: SlicingProfileSummary[] = []): string[] {
...
function extractMachineProfilePrinterModelOptions(profile: SlicingProfileSummary): string[] {
  const fromName = extractPrinterModelLabelFromProfileName(profile.name)
  return [...(profile.printerModels ?? []), ...(fromName ? [fromName] : [])]
}

function extractPrinterModelLabelFromProfileName(value: string): string | null {
...
function isVisibleBambuStudioPrinterModel(value: string): boolean {
...
function looksLikeBambuStudioBaseMachineProfile(value: string): boolean {
  const normalized = normalizedProfileText(value)
  const compacted = compactProfileText(value)
...
function sortPrinterModelOptions(values: string[]): string[] {
  const collator = new Intl.Collator(undefined, { numeric: true, sensitivity: 'base' })
  return [...values].sort((left, right) => {
...
function printerModelSortRank(value: string): number {
  const normalized = normalizedProfileText(value)
...
function formatSlicingProfileMetadata(profile: SlicingProfileSummary): string {
  const parts = [
...
function dedupeVisibleProcessProfiles(
  profiles: SlicingProfileSummary[],
  selectedMachineProfile: SlicingProfileSummary | null,
  model: string
): SlicingProfileSummary[] {
...
function scoreVisibleProcessProfile(
  profile: SlicingProfileSummary,
  selectedMachineProfile: SlicingProfileSummary | null,
  model: string
): number {
...
function matchesSelectedProcessPrimaryPrinterModel(profile: SlicingProfileSummary, model: string): boolean {
...
function matchesSelectedProcessCompatiblePrinter(profile: SlicingProfileSummary, model: string): boolean {
  if (model === 'unknown') return false
  const modelTargets = new Set(printerModelTextCandidates(model))
  return (profile.compatiblePrinters ?? []).some((entry) => profileTextCandidates(entry).some((candidate) => modelTargets.has(candidate)))
}

function matchesSelectedProcessMachineProfile(
  profile: SlicingProfileSummary,
  selectedMachineProfile: SlicingProfileSummary | null,
  model: string
): boolean {
...
function formatPlateTypeLabel(value: string): string {
  return value
    .split(/[_\s-]+/)
    .filter(Boolean)
...
function formatPrinterModelLabel(value: string): string {
                    ...
                    <CardContent>
                      <Stack spacing={1}>
                        <Stack direction="row" spacing={1} alignItems="center" justifyContent="space-between" sx={{ flexWrap: 'wrap', gap: 1 }}>
                          ...
                          <Typography level="body-sm" textColor="text.tertiary">
                            {formatLibraryBytes(version.sizeBytes)}
                          </Typography>
                        </Stack>
...
interface PrinterTrayGroup {
  key: string
  label: string
  trays: PrinterTrayOption[]
}
        ...
        selectedIds.map(async (printerId) => {
          try {
            ...
            const body = {
              ...
              amsMapping: sanitizeMapping(mappings[printerId])
...
function SlotOptionLabel({
  ...
  const remainingDetail =
    hasFilament && tray.remainPercent != null && remainGrams != null
...
/**
 * Trim trailing unset entries off a mapping array. Every used filament
 * must already have a chosen slot before this is called — the submit
 * button is gated on `allMappingsComplete` — so any remaining `-1` is
 * for an unused filament and is safe to drop or clamp to 0.
 */
function sanitizeMapping(mapping: number[] | undefined): number[] | undefined {
  ...
  const trimmed = mapping.slice(0, lastSet + 1)
  return trimmed.map((value) => (value < 0 ? 0 : value))
}
...
function filterTrayGroupsForFilament(
  groups: PrinterTrayGroup[],
  requiredNozzleId: number | null
): PrinterTrayGroup[] {
  ...
  return groups
    .map((group) => ({
      ...group,
      trays: group.trays.filter(
        (tray) => tray.nozzleId == null || tray.nozzleId === requiredNozzleId
      )
    }))
