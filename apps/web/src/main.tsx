...
import { DomainMigrationScreen } from './DomainMigrationScreen'
...
function buildDomainMigrationUrl(targetOrigin: string): string {
  const nextUrl = new URL(targetOrigin)
...
if (browserEnv.domainMigrationTarget) {
  const targetUrl = buildDomainMigrationUrl(browserEnv.domainMigrationTarget)
  setSplashScreenProgress(34, 'Redirecting to the new domain')
  root.render(
    <React.StrictMode>
      <DomainMigrationScreen targetUrl={targetUrl} />
    </React.StrictMode>
  )

  void clearOldOriginState().finally(() => {
