 ...
 *
 * Counterpart: `apps/web/src/components/settings/slicing-presets/` renders these; the profile
...
export type SlicingPresetFacetSelections = Record<string, string[]>

export interface SlicingPresetFacet {
  id: string
  ...
  valuesOf: (profile: SlicingPresetSummary) => string[]
}
...
const MACHINE_FACETS: ReadonlyArray<SlicingPresetFacet> = [
  {
    id: 'printerModel',
    label: 'Printer model',
...
const PROCESS_FACETS: ReadonlyArray<SlicingPresetFacet> = [
  {
...
export const SLICING_PRESET_FACETS: Record<SlicingPresetKind, ReadonlyArray<SlicingPresetFacet>> = {
 ...
 */
export function collectSlicingPresetFacetOptions(
  profiles: ReadonlyArray<SlicingPresetSummary>,
  facets: ReadonlyArray<SlicingPresetFacet>
...
export function countActiveSlicingPresetFacets(selections: SlicingPresetFacetSelections): number {
...
export function findSlicingPresetFacet(kind: SlicingPresetKind, facetId: string): SlicingPresetFacet | null {
  return SLICING_PRESET_FACETS[kind].find((facet) => facet.id === facetId) ?? null
}

export interface SlicingPresetGroup {
  key: string
  label: string
 ...
 */
export function groupSlicingPresetsByFacet(
  profiles: ReadonlyArray<SlicingPresetSummary>,
  facet: SlicingPresetFacet
): SlicingPresetGroup[] {
  const buckets = new Map<string, SlicingPresetGroup>()
  for (const profile of profiles) {
 ...
 */
export function filterSlicingPresetsForKind(
  profiles: ReadonlyArray<SlicingPresetSummary>,
  kind: SlicingPresetKind,
  search: string,
  selections: SlicingPresetFacetSelections
): SlicingPresetSummary[] {
  const facets = SLICING_PRESET_FACETS[kind]
  return filterSlicingPresets([...profiles], search, [kind]).filter((profile) =>
    facets.every((facet) => {
