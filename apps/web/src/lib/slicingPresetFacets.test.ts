...
function facet(kind: Parameters<typeof findSlicingPresetFacet>[0], facetId: string) {
  const found = findSlicingPresetFacet(kind, facetId)
  assert.ok(found, `no ${facetId} facet for ${kind}`)
  return found
}

function profile(overrides: Partial<SlicingPresetSummary> & Pick<SlicingPresetSummary, 'id' | 'kind' | 'name'>): SlicingPresetSummary {
  return { source: 'custom', ...overrides }
}

const MACHINES: SlicingPresetSummary[] = [
...
const PROCESSES: SlicingPresetSummary[] = [
...
const FILAMENTS: SlicingPresetSummary[] = [
...
test('each tab sees only its own kind', () => {
  assert.deepEqual(filterSlicingPresetsForKind(ALL, 'machine', '', {}).map((entry) => entry.id), ['m1', 'm2'])
  assert.deepEqual(filterSlicingPresetsForKind(ALL, 'filament', '', {}).map((entry) => entry.id), ['f1', 'f2'])
})

test('facet options list only values that are actually present', () => {
  const options = collectSlicingPresetFacetOptions(MACHINES, SLICING_PRESET_FACETS.machine)
...
test('values within one facet combine with OR', () => {
  const matched = filterSlicingPresetsForKind(MACHINES, 'machine', '', { nozzle: ['0.4 mm', '0.6 mm'] })
  assert.deepEqual(matched.map((entry) => entry.id), ['m1', 'm2'])
})

test('separate facets combine with AND', () => {
  const none = filterSlicingPresetsForKind(MACHINES, 'machine', '', {
...
test('process layer height comes from the preset name', () => {
  const options = collectSlicingPresetFacetOptions(PROCESSES, SLICING_PRESET_FACETS.process)
  assert.deepEqual(options.layerHeight, ['0.08mm', '0.20mm'])
  const fine = filterSlicingPresetsForKind(PROCESSES, 'process', '', { layerHeight: ['0.08mm'] })
...
test('search narrows within the active kind', () => {
  const matched = filterSlicingPresetsForKind(ALL, 'filament', 'petg', {})
  assert.deepEqual(matched.map((entry) => entry.id), ['f2'])
})

test('grouping buckets by a facet value, sorted', () => {
  const groups = groupSlicingPresetsByFacet(FILAMENTS, facet('filament', 'filamentType'))
...
test('a profile with several values appears in every matching group', () => {
  ...
  const groups = groupSlicingPresetsByFacet([multi], facet('process', 'compatiblePrinter'))
...
test('the active-filter count ignores facets with nothing selected', () => {
  assert.equal(countActiveSlicingPresetFacets({}), 0)
  assert.equal(countActiveSlicingPresetFacets({ nozzle: [], printerModel: ['Bambu Lab H2D'] }), 1)
  assert.equal(countActiveSlicingPresetFacets({ nozzle: ['0.4 mm'], printerModel: ['Bambu Lab H2D'] }), 2)
})
