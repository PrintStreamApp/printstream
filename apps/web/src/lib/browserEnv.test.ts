...
test('parseBrowserEnv preserves the configured domain migration target', () => {
  assert.equal(
    parseBrowserEnv({ VITE_DOMAIN_MIGRATION_TARGET: ' https://printstream.app ' }).domainMigrationTarget,
    'https://printstream.app'
  )
})