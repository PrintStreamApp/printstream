 ...
 * the server has no 3D renderer, so the model-previewer plugin (which owns
...
/** Register the renderer used to preview STL files. Called once by the model-previewer plugin. */
 ...
 * plate PNG). The model-previewer plugin registers one that renders the file at Bambu's
