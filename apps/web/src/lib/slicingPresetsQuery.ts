...
import type { SlicingCapabilities, SlicingPresetsResponse } from '@printstream/shared'
import { apiFetch } from './apiClient'
import { slicingPresetsResponseIsUsable } from './slicingPresetMatching'

/** The catalogue is effectively static per slicer image; keep it fresh for a few minutes. */
export const SLICING_PRESETS_STALE_TIME_MS = 5 * 60_000
 ...
 */
const SLICING_PRESETS_STALL_TIMEOUT_MS = 25_000

export function slicingPresetsQueryOptions(targetId: string) {
  return {
    ...
    queryFn: async ({ signal }: { signal?: AbortSignal }) => {
      ...
      const stallTimer = setTimeout(() => {
        stalled = true
        controller.abort()
      }, SLICING_PRESETS_STALL_TIMEOUT_MS)
      let result: SlicingPresetsResponse
      try {
        result = await apiFetch<SlicingPresetsResponse>(`/api/slicing/profiles?${params.toString()}`, { signal: controller.signal })
      } catch (error) {
    ...
    staleTime: SLICING_PRESETS_STALE_TIME_MS,
    retry: 5,
 ...
 */
export function prefetchSlicingPresets(queryClient: QueryClient, capabilities: SlicingCapabilities | null | undefined): void {
  ...
  if (!targetId) return
  void queryClient.prefetchQuery(slicingPresetsQueryOptions(targetId))
}
