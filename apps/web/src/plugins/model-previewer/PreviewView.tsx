...
export function PreviewView(props: Record<string, unknown>) {
  ...
  const platesQuery = useQuery({
    ...
    enabled: Boolean(open && fileId && previewMode !== 'stl'),
    staleTime: 60_000,
  ...
  const sceneQuery = useQuery({
    ...
    enabled: Boolean(open && fileId && previewMode === '3mf'),
    staleTime: 60_000,
    refetchOnMount: 'always'
  })
  ...
  useEffect(() => {
    ...
    if (
      fileQuery.isLoading
      || (previewMode !== 'stl' && platesQuery.isLoading)
      || (isThreeMfScene && sceneQuery.isLoading)
    ) {
    ...
    if (previewMode === 'stl') {
      ...
      void fetch(buildApiUrl(`${resourceBase}/plate-gcode?plate=${selectedPlate}`), {
        ...
        .then((text) => {
          ...
          attachObject(buildPlateGcodePreviewObject(preview.object))
        })
  ...
  const heading = previewMode === 'stl' ? '3D preview' : '3D plate preview'
  const showPlatePicker = previewMode !== 'stl' && plates.length > 0
...
function resolvePreviewMode(file: LibraryFile | null): 'stl' | '3mf' | 'plate-gcode' | null {
  if (!file) return null
...
function buildPlateGcodePreviewObject(object: THREE.Object3D): THREE.Object3D {
