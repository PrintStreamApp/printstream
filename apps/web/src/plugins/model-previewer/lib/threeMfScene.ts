/**
 * Shared Three.js helpers for the model-previewer plugin.
 *
 ...
 * `model-previewer` plugin directory.
 */
...
/** A small flat numeric tick label lying on the bed plane, dim enough to read as a ruler mark. */
function createAxisTickLabel(text: string, heightMm: number): THREE.Object3D | null {
  ...
  context.font = `500 ${fontSize}px sans-serif`
  context.fillStyle = 'rgba(158, 186, 198, 0.9)'
  ...
  const mesh = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ map: texture, transparent: true, opacity: 0.45, depthWrite: false, side: THREE.DoubleSide })
  )
  return mesh
}
