/**
 * The workspace's `FilamentConfigResolver`: resolves a filament preset's config through the tenant
 * route, for every surface that needs one.
 *
 * OWNS the tenant half of the resolver pair. Its counterpart is `localFilamentResolver.ts` in the
 ...
 */
export const resolveTenantFilamentConfig: FilamentConfigResolver = ({
  filamentProfileId,
  targetId,
  sourceFileId,
  projectFilamentId
