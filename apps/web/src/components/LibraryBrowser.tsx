...
import { getSceneThumbnailProvider, getStlThumbnailProvider } from '../lib/stlThumbnailRegistry'
...
export function LibraryBrowser({
  ...
  const hideMetadataTags = hideMetadataChipsOnMobile && isMobileViewport
  ...
  const resolveDraggedItem = (event: DragEvent<HTMLElement>): LibraryDragItem | null => {
    ...
    try {
      ...
      if (parsed.type === 'file') {
        ...
        const file = files.find((entry) => entry.id === parsed.id)
      ...
      if (parsed.type === 'files') {
        ...
        const draggedFiles = files.filter((entry) => parsed.ids?.includes(entry.id))
...
function serializeLibraryDragItem(item: LibraryDragItem): SerializedLibraryDragItem {
  if (item.type === 'file') {
...
export function FileThumbnail({
  ...
  // STL files have no server thumbnail; a plugin may register a client-side
  // renderer (Three.js lives in the model-previewer plugin, not in core).
  const stlProvider = file.kind === 'stl' ? getStlThumbnailProvider() : null
  const [stlThumbnailUrl, setStlThumbnailUrl] = useState<string | null>(null)
  useEffect(() => {
    if (disabled || !stlProvider) {
      setStlThumbnailUrl(null)
      return
    }
    let cancelled = false
    const controller = new AbortController()
    stlProvider(file, controller.signal)
      .then((url) => { if (!cancelled) setStlThumbnailUrl(url) })
      .catch(() => { if (!cancelled) setStlThumbnailUrl(null) })
    return () => {
      cancelled = true
      controller.abort()
    }
    // Re-render only when the underlying file (id/revision) or gating changes.
  }, [disabled, stlProvider, file, file.id, file.uploadedAt])

  const thumbnailUrl = serverThumbnailUrl ?? stlThumbnailUrl
