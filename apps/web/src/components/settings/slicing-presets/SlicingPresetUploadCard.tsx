...
import { extractErrorMessage, type SlicingPresetResponse } from '@printstream/shared'
import { useMutation, useQueryClient } from '@tanstack/react-query'
import { ApiError, apiFetch } from '../../../lib/apiClient'
import { usePromptDialog } from '../../PromptDialogProvider'
import { buildSlicingPresetUpload } from './slicingPresetUpload'

export function SlicingPresetUploadCard(): JSX.Element {
  ...
  const uploadProfile = useMutation({
    ...
    mutationFn: async ({ file, overwrite }: { file: File; overwrite?: boolean }) =>
      await apiFetch<SlicingPresetResponse>('/api/slicing/profiles', {
        method: 'POST',
        body: await buildSlicingPresetUpload(file, overwrite)
      }),
    onSuccess: async () => {
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} justifyContent="space-between" alignItems={{ xs: 'stretch', sm: 'flex-end' }}>
            <Stack spacing={0.5}>
              ...
              <Typography level="body-sm" textColor="text.tertiary">
                Upload `.json`, `.bbscfg`, `.bbsflmt`, or preset `.zip` exports. Preset kinds are auto-detected from the file using BambuStudio's own preset rules.
              </Typography>
            </Stack>
            {/* Never wraps: the label is two words and the description beside it already takes the
                slack, so letting the button shrink turned it into a two-line block. */}
            <Button
              loading={uploadProfile.isPending}
