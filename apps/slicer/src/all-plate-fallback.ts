/**
 * All-plate `.gcode.3mf` export fallback: merges per-plate slices into a single
 * all-plate output for printer models that lack a working `--slice 0` direct
 * export (everything but H2D/H2DPRO/H2C). Stitches each plate's
 * `slice_info.config` / `model_settings.config` blocks and `plate_N.*` assets
 * back into one package, and backfills missing `plate_N.png` model thumbnails
 * from the slicer input. Thumbnail backfill is best-effort and never throws.
 */
