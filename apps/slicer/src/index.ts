...
import { z } from 'zod'
import { createSlicingJobSchema, isDirectPrintableFileName, slicingProfileKindSchema, type SlicingOutputLine } from '@printstream/shared'
...
const sliceProfileFileSchema = z.object({
...
const sliceEnvelopeSchema = z.object({
  ...
  try {
    ...
    if (parsed.data.source === 'builtin') {
      const builtinContent = await readFile(
        path.join(target.profileDir, 'process_full', `${sanitizeProfileFileName(parsed.data.name)}.json`),
        'utf8'
      )
      record = await resolveCustomProfileConfig(builtinContent, 'process', target.profileDir)
    } else {
      if (!parsed.data.content) {
        response.status(400).json({ error: 'Custom process profile is missing content' })
        return
      }
      record = await resolveCustomProfileConfig(parsed.data.content, 'process', target.profileDir)
    }
    response.json({ config: record })
  } catch (error) {
    response.status(404).json({ error: (error as Error).message || 'Process profile not found' })
  }
})
...
async function runCli(input: {
  ...
  plate: number
  profileFiles: z.infer<typeof sliceProfileFileSchema>[]
 ...
 */
async function runEstimateModeMachineSwitch(input: {
  slicerTarget: RuntimeSlicerTarget
  inputPath: string
  outputPath: string
  outputFileName: string
...
async function getSupportedCliFlags(
  ...
  const helpText = await new Promise<string>((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      reject(new Error(stderr.trim() || stdout.trim() || `Slicer CLI help exited with code ${code ?? 'unknown'}`))
    })
  })
...
async function executeCli(input: {
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      child.stdout.setEncoding('utf8')
      child.stderr.setEncoding('utf8')
      child.stdout.on('data', (chunk: string) => appendOutput(input.outputLines, 'stdout', chunk))
      child.stderr.on('data', (chunk: string) => {
...
async function readDisplayProfile(filePath: string, kind: 'machine' | 'process' | 'filament', profileDir: string) {
  try {
...
function extractProfileMetadata(record: Record<string, unknown>) {
  // BambuStudio profile JSON is authoritative for profile metadata. The web
  // dialog only falls back to name parsing when exported/project profiles omit it.
  return omitEmptyMetadata({
    ...
    // Machine profiles declare the presets BambuStudio falls back to when the
    // current preset is incompatible with the selected printer. The web slice
    // dialog uses these to mirror that fallback on a cross-model machine switch.
...
function omitEmptyMetadata<T extends Record<string, string | string[] | number[] | undefined>>(metadata: T): Partial<T> {
...
async function materializeProfileFile(
  profile: z.infer<typeof sliceProfileFileSchema>,
  outputDir: string,
  profileDir: string,
...
function buildBuiltinProfileId(kind: 'machine' | 'process' | 'filament', name: string): string {
...
type SlicingMetadataFields = {
  estimatedPrintTimeSeconds?: number | null
  estimatedPrepareTimeSeconds?: number | null
  estimatedFilamentLengthMm?: number | null
  estimatedFilamentWeightGrams?: number | null
  estimatedFilamentCost?: number | null
  materials?: SlicingMaterialUsage[] | null
}
