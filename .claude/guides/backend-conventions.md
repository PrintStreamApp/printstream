...
Read this when working in API or shared contract code (`apps/api/**`, `packages/shared/src/**`).
...
- Return standard `429` responses with `Retry-After` for rate-limited clients, and advertise the budget via `RateLimit-Limit`/`RateLimit-Remaining`/`RateLimit-Reset` headers (most-restrictive limiter wins) so bursty clients can self-pace. Never log request bodies, auth codes, tokens, LAN access codes, or other secrets while handling throttled traffic.
