...
| `LICENSE_REFRESH_ORIGIN` | `https://printstream.app` | Where a subscription-backed license key is refreshed. Only keys tied to a live Pro subscription carry an expiry and refresh at all: a community or Lifetime key is perpetual and never contacts this origin, so a personal or air-gapped install makes no outbound request. The refresh sends the key and nothing else (no printer counts, no telemetry), and a failed refresh is retried silently, since the key's multi-week window (not the network) decides when access ends. Override only to point a staging deployment at sandbox billing. |
