...
- **Offline use.** The service worker is what serves the app with no network, and it only registers on a secure origin. Over HTTP the app needs the server reachable to load. (Automatic updates are *not* affected: clients compare the build they are running against the one the server reports, over the WebSocket and an API response header, so open tabs still refresh themselves onto a new version after an upgrade.)
