...
| --- | --- | --- |
| **Editor** | web | `apps/web/src/plugins/model-previewer/` — `EditorView.tsx` (3D editor), `lib/editorModel.ts` (the editable scene model + `buildSceneEdit`), `lib/threeMfScene.ts` (scene→Three.js), `lib/editorImports.ts`, `lib/meshCut.ts` (Cut tool: plane cut + capped halves staged as imports) |
...
- `apps/web/src/pages/LibraryView.tsx` (~8k lines) → extract `SliceSettingsController` +
  ...
  browser.
- `apps/web/src/plugins/model-previewer/EditorView.tsx` (~4k lines) → extract gizmo modes,
