...
- **Cloud support is optional.** On a commercial license with current support, Help & Feedback can send a two-way conversation with attachments and Suggestions can open the shared product board. Help & Feedback explains its payload inline, and the plugin settings describe both surfaces. The connection does not upload printer, library, print-history, or camera data, and disabling the plugin restores the email support fallback.
