...
cd printstream
cp .env.server.example .env        # edit if you like; defaults work for a LAN install
cp compose.server.example.yml compose.yml
docker compose --profile bridge up -d --build
```

Then open `http://<your-server>:8080`. A default workspace is created automatically on first start — add your printers and go.

- Printers must have **LAN mode** enabled with their access code handy (Bambu printer screen → Settings → LAN Only Mode / access code).
- If your printers are on a different network than the server, run the bridge near the printers instead: copy `.env.bridge.example` to `.env` on that machine, point `BRIDGE_SERVER_URL` at your server, and `docker compose -f compose.bridge.example.yml up -d --build`.
- Full details, reverse-proxy notes, and the optional slicer service: [docs/deployment.md](docs/deployment.md). All settings: [docs/configuration.md](docs/configuration.md).

## Plugins

Optional features ship as plugins you can switch on per workspace: sign-in providers (passkeys/email codes, OAuth/OIDC), the 3D previewer/editor, server-side slicing helpers, production orders, notifications (browser push, Discord, ntfy), firmware updates, plate-clearing confirmation, and the Home Assistant bridge. Third-party plugins can be uploaded from the plugin manager.
...
- [docs/open-core.md](docs/open-core.md) — how this public repo relates to the hosted product
