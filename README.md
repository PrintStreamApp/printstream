...
<a href="https://apps.microsoft.com/detail/9N2CJ8BB9RTZ"><img alt="Download from Microsoft Store" src="apps/web/public/store-badges/microsoft-store.svg" width="161"></a>
...
- **Jobs & stats**: print history with snapshots, success rates, print hours, and filament usage per printer and per workspace. Choose a date range for Stats, compare reliability by printer, model, and recorded material, and re-dispatch finished prints as-is or slice them again from their source project.
...
> **Serving over plain HTTP?** Everything printer-related works, but browsers reserve some features for HTTPS origins: browser push notifications, installing PrintStream as an app (PWA), offline use, and passkey sign-in all need HTTPS (`http://localhost` counts as secure, so a same-machine install is unaffected). To get the full experience on a network address, put a TLS reverse proxy in front; see [Reverse proxy and HTTPS](docs/deployment.md#reverse-proxy-and-https).

## Plugins

Optional features ship as plugins you can switch on per workspace: sign-in providers (passkeys/email codes, OAuth/OIDC), the 3D model studio (project editor + previews), server-side slicing helpers, filament calibration (pressure-advance towers and flow-ratio plates), production orders, notifications (browser push, Discord, ntfy), firmware updates, printer maintenance reminders (when each model is next due for lubrication and other servicing Bambu recommends), plate-clearing confirmation, importing models from a URL (paste a MakerWorld link or a direct file link), and the Home Assistant bridge. Any valid key can use the shared Suggestions board through the optional PrintStream Cloud connection; commercial keys with current or grandfathered support can also use in-app Help. The connection sends nothing until someone uses one of those surfaces, emails unread support replies to the licence owner, and can be disabled from the plugin manager. Third-party plugins can be uploaded from the plugin manager.

## Good to know

- **This edition needs a license key.** Self-hosting for personal, non-commercial use is free: request a [community key](https://printstream.app/community-license) and paste it under Settings → License. New installs get a 30-day grace period, after which adding printers and starting prints pause until a key is entered. Nothing else is affected, existing printers keep working, and your data is never locked away. A community key is perpetual; cloud contact occurs only when someone uses an optional cloud feature. Business use needs a Pro subscription (which covers self-hosting at no extra cost) or a one-time Lifetime license.
- **Cloud features are optional.** Any valid key can open the shared Suggestions board. A commercial key with current or grandfathered support can also send two-way Help conversations with attachments. Help & Feedback explains its payload inline, and the plugin settings describe both surfaces. The connection does not upload printer, library, print-history, or camera data, and disabling the plugin restores the email support fallback.
