...
export const bridgeReleaseSchema = z.object({
...
export const bridgeReleaseChannelSchema = z.object({
  latestVersion: z.string().min(1),
  minimumSupportedProtocol: z.number().int().nonnegative(),
  minimumRecommendedVersion: z.string().min(1),
  releases: z.array(bridgeReleaseSchema)
})

export type BridgeRelease = z.infer<typeof bridgeReleaseSchema>

export const bridgeReleaseManifestSchema = z.object({
  schemaVersion: z.literal(1),
  generatedAt: z.string().datetime(),
  channels: z.record(bridgeReleaseChannelSchema)
})
