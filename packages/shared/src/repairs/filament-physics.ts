 ...
 * width matching neither `slots` nor the project's DECLARED variant-row count, flags the project.
 * Note "declared", not `slots x variants`: the blocks are not equal width (a TPU slot owns every
 * printer variant), and multiplying convicted healthy projects at their own correct width.
 *
 * REPAIRED BY SAVING, and unlike its siblings not by the staged repair step either. (No repair has
 * a server-side route: one existed and was deliberately removed so repairs stay explicit and
 * user-driven.) `rebindProjectFilamentPhysics` cannot do it: it
...
import { filamentVariantRowCount, isFilamentVariantOption } from '../variant-options.js'
 ...
 * `slots`, the project's TOTAL variant-row count for a variant-scoped key, or a length-1 broadcast
 * (a single value applying to every slot, measured on real printer-written files). Shared by this
 * inspector's stale-width check and the restore's preserve-present rule
 * (`restore-filament-physics.ts`), so a key can never be counted healthy by one and rewritten, or
 * left flagged, by the other.
 *
 * `variantRowCount` is what the project DECLARES (`filament_extruder_variant`'s length), never
 * `slots x floor(rows / slots)`. The blocks are not equal width, a TPU slot owns every printer
 * variant while its neighbours share the standard ones, so the old multiplication convicted a
 * healthy PLA + TPU project (5 rows across 2 slots) at its own correct width. Counting rows needs no
 * per-slot split: however they are distributed, one value per row is the rule.
 */
export function isAcceptableFilamentValueWidth(
  ...
  if (length === 1 || length === slotCount) return true
  if (!isFilamentVariantOption(key) || variantRowCount <= 0) return false
  return length === variantRowCount
}
...
export interface FilamentPhysicsInspection {
  ...
  missingKeys: string[]
  /**
   * Filament-option arrays whose length matches neither `slots` nor the declared variant-row count: stale
...
export function inspectProjectFilamentPhysics(
  ...
  const variantRowCount = filamentVariantRowCount(record)
  ...
  for (const key of widthJudged) {
    ...
    if (!isAcceptableFilamentValueWidth(key, value.length, slotCount, variantRowCount)) staleWidthKeys.push(key)
  }
