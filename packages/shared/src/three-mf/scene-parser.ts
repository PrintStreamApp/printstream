...
/**
 * Build the plated scene for `plateIndex`, falling back to the project's first plate when that
 * index is absent.
 *
...
function parsePrimeTower(projectSettingsJson: string | null, plateIndex: number): ThreeMfPrimeTower | null {
  ...
  // BambuStudio's own defaults for a project that names no tower position
...
function parsePrimeTowerSizing(record: Record<string, unknown>): ThreeMfPrimeTowerSizing {
  ...
  const spiralMode = parseConfigBoolean(record.spiral_mode)
  return { wipeVolume, layerHeight, infillGap, ribWall, ribWidth, extraRibLength, extruderCount, needWipeTower, spiralMode }
}
