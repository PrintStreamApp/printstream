 ...
 * (multi-instance objects contribute one part per copy), keeping the parts' as-authored arrangement
 * but re-centred as a group on the XY origin at Z=0 (see {@link recentreParts}), so the editor can
 ...
 * settings, plates beyond the first) is deliberately dropped: this is a geometry import, not a
 * project merge. With `objectId`, only that object's first instance is extracted, in object-local
 ...
 * `apps/web/src/plugins/model-studio/lib/localImportStore.ts` (in-tab archive).
 */
import { isNonRenderableThreeMfPartSubtype } from '../three-mf-part-subtype.js'
import { MAX_IMPORT_TRIANGLES } from './mesh-stl.js'
...
/**
 * How the extractor reads the archive. Both methods may throw for a file with no Bambu metadata;
 * that is how the vanilla fallback is detected, so do NOT swallow parse failures into empty results.
 */
export interface ThreeMfImportSource {
  /**
   * UTF-8 text of a ZIP entry, or null when the entry is ABSENT.
   *
  ...
  /** Plate numbers present in the file, in file order. */
  readPlateIndexes(): Promise<number[]>
  /** One plate's parsed scene. */
  readScene(plateIndex: number): Promise<ThreeMfScene>
}
...
/**
 * Extract a 3MF's printed geometry as a staged-import mesh. Throws {@link ThreeMfImportError} with
 * a user-facing message when the file holds no importable geometry or exceeds the import triangle
 * cap. `objectId` narrows the extraction to one object (Bambu object id).
 */
export async function extractThreeMfImportMesh(
  source: ThreeMfImportSource,
  options?: { objectId?: number }
): Promise<ImportedMesh> {
  const sources = await resolvePartSources(source, options?.objectId)
  if (sources.length === 0) {
    throw new ThreeMfImportError(options?.objectId != null
  ...
  for (const partSource of sources) {
    const mesh = await extractComponentMesh(source, entryXmlCache, partSource.entryPath, partSource.objectId, partSource.transform, 0)
    if (!mesh || mesh.indices.length === 0) continue
    totalTriangles += mesh.indices.length / 3
    if (totalTriangles > MAX_IMPORT_TRIANGLES) {
      throw new ThreeMfImportError('This 3MF has too many triangles to import.')
    }
    const base = partSource.name?.trim() || `Part ${parts.length + 1}`
    const count = (nameCounts.get(base) ?? 0) + 1
    nameCounts.set(base, count)
    parts.push({ name: count === 1 ? base : `${base} (${count})`, mesh, subtype: partSource.subtype })
  }
  if (parts.length === 0) {
 ...
 * helper volumes (they must keep their position relative to the model they act on). Letting a
...
/**
 * Resolve which (entryPath, objectId, transform) tuples make up the import. Prefers the Bambu-aware
 * scene parse (plate assignment + part subtypes); falls back to a plain root-model build parse for
 * vanilla 3MFs without Bambu metadata.
 */
async function resolvePartSources(source: ThreeMfImportSource, objectId?: number): Promise<ExtractedPartSource[]> {
  try {
...
async function resolveSceneSources(source: ThreeMfImportSource, objectId?: number): Promise<ExtractedPartSource[]> {
  const plateIndexes = await source.readPlateIndexes()
...
async function resolveVanillaSources(source: ThreeMfImportSource, objectId?: number): Promise<ExtractedPartSource[]> {
  const rootXml = await source.readEntryText('3D/3dmodel.model')
  ...
  // A build-less (or empty-build) file still has geometry worth importing: fall back to every
  // mesh-bearing object at its authored coordinates.
  if (sources.length === 0) {
...
async function extractComponentMesh(
  ...
  if (xml == null) {
    const text = await source.readEntryText(entryPath)
  ...
  for (const match of block.matchAll(/<component\b([^>]*)\/>/g)) {
    ...
    const child = await extractComponentMesh(source, entryXmlCache, childEntry, childId, composed, depth + 1)
