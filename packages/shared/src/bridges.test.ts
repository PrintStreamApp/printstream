...
test('bridgeUpdateBlocksPrinting blocks only incompatible statuses', () => {
  const blocking: BridgeUpdateStatus[] = ['updateRequired', 'imageUpdateRequired', 'runnerUpdateRequired', 'unsupported']
  const allowed: BridgeUpdateStatus[] = ['unknown', 'current', 'updateAvailable', 'updateRecommended']
...
test('bridgeUpdateSupportsInAppUpdate excludes image/runner updates (which need an operator restart)', () => {
  ...
  const selfUpdatable: BridgeUpdateStatus[] = ['updateAvailable', 'updateRecommended', 'updateRequired', 'unsupported']
