...
function openCameraSocket(printer: Printer, signal?: AbortSignal): Promise<TLSSocket> {
  return new Promise((resolve, reject) => {
    ...
    const onAbort = () => onError(createAbortError('The operation was aborted'))
...
/**
 * Resolves the ffmpeg executable to spawn. Standalone bridge builds extract a
 * bundled ffmpeg and point BRIDGE_FFMPEG_PATH at it before the runtime starts;
 * Docker and dev runs rely on ffmpeg being on PATH.
 */
function ffmpegCommand(): string {
  return process.env.BRIDGE_FFMPEG_PATH?.trim() || 'ffmpeg'
}

function hasFfmpeg(): boolean {
  if (hasFfmpegCache === true) return true
  try {
    hasFfmpegCache = spawnSync(ffmpegCommand(), ['-version'], { stdio: 'ignore' }).status === 0
  } catch {
...
async function fetchRtspSnapshot(printer: Printer, signal?: AbortSignal): Promise<Buffer> {
  ...
  if (lastError) {
    console.warn(
      `[camera:rtsp:${printer.id}] snapshot failed; all ${urls.length} candidate(s) exhausted: ${lastError.message}`
    )
  }
...
async function* streamRtspFrames(printer: Printer, signal?: AbortSignal): AsyncGenerator<Buffer, void, void> {
  ...
  if (lastError) {
    console.warn(
      `[camera:rtsp:${printer.id}] stream failed; all ${urls.length} candidate(s) exhausted: ${lastError.message}`
    )
  }
  throw lastError ?? new Error(`Camera not supported for model ${printer.model}`)
}
...
function createFtpPauseMonitor(printerId: string, model: Printer['model']): {
  ...
  const unsubscribe = onFtpActivityChange(printerId, (active) => {
    ...
    triggered = true
    controller.abort(createAbortError('The operation was aborted'))
  })
...
function spawnFfmpeg(args: string[]): FfmpegProcess {
  try {
    return spawn(ffmpegCommand(), args, { stdio: ['ignore', 'pipe', 'pipe'] })
  } catch (error) {
