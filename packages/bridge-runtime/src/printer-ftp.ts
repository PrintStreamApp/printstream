  ...
  return enqueuePrinterFtp(printer, async () => {
    ...
    try {
      ...
      try {
        ...
        if (options.signal?.aborted) throw createAbortError('The operation was aborted')
        throw error
      } finally {
      ...
      for (const remote of candidates) {
        for (const variant of buildPrinterPathVariants(remote)) {
          ...
          try {
            ...
            // Try the next path variant; a missing file is expected when probing
            // root vs /cache/ candidates, but keep the last error so an
            // unreachable printer isn't silently reported as "not found".
            lastError = error
          }
        }
      }
      if (lastError) {
        console.warn(`[printer-ftp] downloadFileFromPrinter: all candidates failed: ${(lastError as Error).message}`)
      }
      return null
    } finally {
