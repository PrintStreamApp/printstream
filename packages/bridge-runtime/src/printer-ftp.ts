 ...
 */
// `connectForPassiveTransfer` is a runtime export of the transfer
// submodule but is not surfaced from the package root, so we reach in
// directly via createRequire (this module runs as ESM).
const nodeRequire = createRequire(import.meta.url)
const { connectForPassiveTransfer, parsePasvResponse } = nodeRequire('basic-ftp/dist/transfer.js') as {
  connectForPassiveTransfer: (host: string, port: number, ftp: FTPContext) => Promise<void>
  parsePasvResponse: (message: string) => { host: string; port: number }
}
