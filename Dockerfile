...
# App image identity, surfaced in the web footer. VERSION is the product SemVer,
# REVISION is the exact git commit, and PUBLISHED is "true" only for the open-core image pushed to
...
ARG PRINTSTREAM_IMAGE_VERSION=unknown
...
RUN printf '{"version":"%s","revision":"%s","published":"%s"}\n' "$PRINTSTREAM_IMAGE_VERSION" "$PRINTSTREAM_IMAGE_REVISION" "$PRINTSTREAM_IMAGE_PUBLISHED" > app-build-metadata.json
