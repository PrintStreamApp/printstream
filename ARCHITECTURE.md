...
The web root route goes straight into the app: protected routes resolve auth/bootstrap state and route actors to the platform workspace, workspace chooser, or workspace as appropriate. Workspace stats remain available at `/workspaces/<slug>/stats`. If sign-in is required for a protected route, the auth UI renders above the normal shell at the URL the user originally requested instead of redirecting the browser to a separate auth route first.
...
Printer --MQTT--> printer-manager --> printerEvents bus --> ws-server --> /ws --> web
                                              ...
                                              |                         +--> WorkspaceStats / PrinterStats / PlatformStats rollups
...
Server-side slicing for unsliced Bambu Studio `.3mf` files is routed through one or more separate, identical slicer runtimes/containers (`SLICER_SERVICE_URL` accepts a comma-separated list; jobs go to the least-busy instance, one concurrent slice per instance by default) rather than the API or bridge process. The API validates workspace-owned source files, exposes workspace-scoped custom BambuStudio preset persistence (managed from the editor's settings dialog, not Settings > Slicing, which now covers only workspace slicing behaviour), merges those uploaded printer/process/material presets with slicer-target built-ins when populating the slice dialog, lets users choose either a real printer as a slicer-context preset or a manual profile, enforces bounded FIFO queueing, persists saved slice results back into the visible library, stores print-flow slice outputs as hidden transient library artifacts so the follow-on print can use them without cluttering the library browser, replaces an existing same-name file (with version archiving) when a kept output is saved over it, and snapshots job-owned thumbnail media so slicing history survives later library cleanup or manual source-file deletion. Before handing the project over it also authors that slice's own settings into it — the chosen process preset, each slot's filament preset, the per-slice and per-material overrides, and the plate type, none of which otherwise enter the 3MF — and then preserves the resulting project as a hidden content-deduped snapshot linked from the sliced output and from the print-history row. That is what lets a finished print be sliced AGAIN from the project it came from rather than only re-dispatched as the same G-code, including for editor slices whose project was never saved to the library. The preserved project only earns its keep if a print actually started: the discard path and a dedicated cleanup pass remove any snapshot no job or kept output still references. The slicer runtime owns BambuStudio CLI execution, output capture, cancellation, and artifact generation, including materializing selected built-in or custom preset JSON files and post-processing packaged `.gcode.3mf` outputs so saved artifact metadata reflects the selected printer/process/material target rather than stale source-project metadata. It runs on the PrintStream server alongside the API, not next to the bridge; the bridge runtime itself does not absorb slicer responsibilities.
...
- Plugins declare `runtimeSurfaces` (`platform`, `workspace`, or both), `managerSurfaces`, and `workspaceAccess` so the host can keep platform-only features out of workspaces and vice versa.
...
- Plugins that run on the platform surface carry an independent platform-workspace enable toggle (`platformEnabled` in the catalog; for dual-surface workspace-controlled plugins it is stored separately from the workspace-default flag).

The platform-facing plugin APIs are split intentionally:

- `/api/admin/plugins` is the privileged platform manager for install/uninstall and workspace-availability policy.
- `/api/plugin-catalog` is the current-workspace catalog used by the web shell to decide which routes, slots, and workspace plugin toggles are available without exposing the full admin API.
...
The named extension points core currently exposes are: `printer.card.actions`, `printer.card.headerChips`, `printer.card.menuItems`, `printer.card.dialogs` (printer cards); `library.create`, `library.fileActions`, `library.overlays` (library); `jobs.sections` (full-width sections above the core Jobs-page sections, e.g. the print-queue backlog; the context's `registerSection` callback lets a section add itself to the page's section nav); `stats.cards` (workspace stats grid); `slicing.editor` (slice dialog); `shell.footer` (app-shell footer actions); `shell.background` (headless app-level state syncers, e.g. unread tab badges); `shell.overlays` (app-level overlay dialogs, e.g. the auth-local post-sign-in passkey setup offer); `help.conversations` (existing support conversations inside the core Help & feedback dialog); `account.billing`, `account.support` (workspace Account page sections); and the auth-provider slots `auth.signIn`, `auth.recentVerification`, `auth.userManagement.credentials`, `auth.userManagement.lifecycle`, `account.security`, `settings.authenticationProviders`, `settings.authenticationSetup`. Adding a new point means rendering `<PluginSlot name="..." context={...} />` (or `webPluginRegistry.slots(name)`) in the host and passing a free-form context, including callbacks, so the plugin can drive host state without core depending on the plugin.
...
| `home-assistant` | API + web | Publishes a printer + AMS bootstrap snapshot with cover/camera paths plus live plugin events for the companion Home Assistant custom integration, and adds a setup guide surface in the web app with Lovelace card examples plus Home Assistant-side image/media surfaces. The HA package bootstraps from the snapshot, follows `/ws` updates in real time, registers printer/AMS control services, supports multiple Home Assistant config entries against different workspaces, and turns that data into model-aware printer, AMS, tray, external-spool, and image entities without registering unsupported capabilities on incompatible hardware. |
| `notifications-browser` | API + web | Web Push via VAPID keypair (auto-generated). The service worker (`push-handler.js`) fires OS notifications even when the app is closed. Enablement is per workspace, per device: the browser's single push subscription is registered with each workspace the user enables it in, so enabling one workspace never disturbs another workspace's delivery. |
| `notifications-discord` | API + web | Posts notifications to Discord webhooks via a per-scope recipients list: shared entries receive the workspace's broadcast events, self-bound personal entries receive only that user's targeted messages. A pre-list `webhookUrl` setting keeps working as an implicit shared entry. |
| `notifications-email` | API + web | Emails notification events to opted-in users (per-workspace opt-in; opted-in platform users at the platform scope) through the core email-transport registry. User-targeted messages mail the target users' account addresses directly (transactional semantics), and messages whose emitter already sent its own email are skipped. |
| `notifications-ntfy` | API + web | Forwards notifications to ntfy topics via the same per-scope recipients list as the Discord channel (shared topics for broadcasts, self-bound personal topics for targeted messages; legacy `topicUrl` reads as an implicit shared entry; managed-bridge installs keep the `NTFY_TOPIC_URL` broadcast fallback). |
| `email-smtp` | API + web | Self-hosted/OSS SMTP transport: points the core email-transport registry at an operator-supplied SMTP server so email-backed features deliver without a cloud provider. |
| `orders` | API + web | Production order templates with named variants, per-copy filament-color override snapshots, required-print expansion, tracked per-print lifecycle, and an app-relative `/orders` workflow mounted under a workspace slug. Template items may reference unsliced project 3MFs; starting one slices first (web slice-then-print flow) and dispatches the sliced output via `slicedFileId` while the item keeps pointing at the source 3MF. |
...
| `/api/stats` | Workspace setup-readiness and high-level workspace production metrics for the stats page |
...
| `/api/editor` | 3D plate-editor support: staging imported STL/STEP/3MF geometry, persisting `SceneEdit` project edits, and baking single-object 3MF exports as downloads |
| `/api/camera` | Camera snapshots and stream relay |
| `/api/logs` | Platform or workspace log viewer and clearing, combining durable audit entries with in-memory system diagnostics |
...
| `/api/bridges` | Workspace bridge management: listing plus the connect/rename flows that attach dormant bridge records to a workspace |
...
| `/api/admin/plugins` | Platform-only plugin listing, install/uninstall, enable/disable, and workspace availability policy |
| `/api/plugin-catalog` | Current-workspace plugin catalog used by the web shell and workspace settings |
...
| Model | Purpose |
|---|---|
| `Workspace` | Hosted workspace directory and routing identity. The API ensures a canonical `Default` workspace exists for owner-mode installs, but platform mode remains workspaceless until a selection is made. |
...
| `PrinterView` | Saved Printers-page view definitions per workspace (printer subset, cards-per-row, state/model/nozzle/plate filters, sort, and per-card content settings) surfaced through `/api/printer-views`. |
...
| `WorkspaceStats` | Workspace-scoped lifetime production totals maintained by database triggers when print jobs finish. Includes total/success/failed/cancelled print counts, successful and wasted print durations, and filament totals only for jobs whose usage can be resolved. |
| `PrinterStats` | Per-printer lifetime totals keyed by workspace and printer serial, so stats survive deleting and re-adding the same physical printer. These are recorded once at job finish by the print-job recorder. |
| `PlatformStats` | Singleton platform-wide rollup maintained from workspace stats and workspace membership/printer counts for efficient platform overview loading. |
| `LibraryFile` | Metadata for the current visible library entry. API-owned bytes live on disk under `LIBRARY_DIR`; bridge-owned bytes live on the owning bridge, with API-side derived metadata/thumbnail caches for fast browsing. Supports a `hidden` flag for transient uploads plus retained deduplicated snapshot rows used to keep dispatched-job reprints stable and to hold the project a sliced output was produced from (`sourceProjectFileId`/`sliceSettingsJson`, mirrored onto `PrintJob` at dispatch). Soft deletes set `deletedAt` (recycle bin; restorable until the retention window expires), `origin` tags the row lifecycle ('upload'/'slice'/'scaffold'/'snapshot') for cleanup windows, and `createdBy*`/`restoredFromVersionNumber` record version attribution and restore provenance. Same-name uploads within the same folder now overwrite this current row in place so existing references stay stable. |
...
| `AuditLog` | Durable audit trail for mutating requests, including actor, workspace, action summary, resource, and status code. Platform logs and workspace logs both read from this table. |
| `AuthUser` / `AuthWorkspaceMembership` / `AuthGroup` | Human auth identities use one global user record per email, workspace-scoped memberships carry login disablement and workspace access, and groups remain workspace-scoped reusable role definitions. |
...
Stats are durable lifetime counters rather than a live scan of history rows. `WorkspaceStats` and `PlatformStats` are updated by PostgreSQL triggers, while `PrinterStats` is claimed atomically from the recorder so retries cannot double-count a finished job. Print-job deletion and printer removal do not subtract already-recorded production totals.
...
Plugin-specific state should live in the `Setting` table by default (keys namespaced `plugin:<name>:<key>`). **Built-in plugins** may add their own workspace-scoped tables to the core schema when the data is structured and relational rather than key/value — the `orders` plugin does this — provided each model carries a `workspaceId` relation to `Workspace`, is listed in `WORKSPACE_SCOPED_MODELS` (`apps/api/src/lib/prisma.ts`), and ships a migration. **Third-party (uploaded) plugins** cannot migrate the core schema and must persist through the `Setting` store.
...
| Workspace stats | Workspace stats page with setup readiness, quick-start tasks, and persisted workspace production totals. Workspaces now land on Printers by default, with stats available at `/workspaces/<slug>/stats`. |
...
| Jobs | One in-progress section that combines queued dispatches and active prints backed by unfinished persistent jobs plus live status and live camera/snapshot media, and chronological print plus slicing history with reprint and slice-again shortcuts plus durable job-owned media that can survive later library cleanup. |
...
| Settings | Workspace-scoped configuration, including local auth access management, workspace plugin toggles, notifications, and workspace logs. |
...
- The API uses `helmet` defaults but disables CORP (it proxies binary streams — camera, library downloads) and replaces helmet's strict CSP with its own policy (`apps/api/src/lib/content-security-policy.ts`) that allows the `blob:`/`data:` camera and thumbnail paths. The policy ships report-only until `CSP_ENFORCE=true`; browsers POST violations to the built-in `/api/csp-report` sink either way, and `CSP_ANALYTICS_ORIGIN` can allow-list a first-party analytics tracker. See `docs/configuration.md`.
