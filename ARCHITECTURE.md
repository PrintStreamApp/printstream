...
The web root route is the public marketing homepage, with public legal, contact, and open-source-license routes at `/terms`, `/privacy`, `/contact`, and `/licenses`. Authenticated users can open the application from the public pages, while protected app routes still resolve auth/bootstrap state and route actors to the platform workspace, workspace chooser, or tenant workspace as appropriate. Tenant stats remain available at `/workspaces/<slug>/stats`. If sign-in is required for a protected route, the auth UI renders above the normal shell at the URL the user originally requested instead of redirecting the browser to a separate auth route first.

`apps/api` is an Express server that owns:

- The cloud/control-plane side of printer connectivity, including direct local MQTT sessions for non-bridged printers and bridge-session ingestion for printers managed through a remote bridge.
...
`apps/bridge` is an outbound runtime that lives on the printer LAN when the API/web stack is hosted elsewhere. It owns SSDP auto-discovery, printer-local MQTT/FTPS/camera transport for bridged printers, bridge-local library copies, and a single outbound bridge session back to the API. Bridge-hosted library files keep their full bytes on the bridge by default; the API caches small derived artifacts such as parsed 3MF plate metadata and thumbnail PNGs under its library data directory so repeated library browsing does not repeatedly cross the slow bridge session. The API only materializes a full bridge file copy locally when a path truly needs local bytes, such as print/slice preparation or fallback parsing after bridge-side metadata extraction is incomplete. Because library files are bridge-owned by default, the bridge's 3MF parser (`apps/bridge/src/library-3mf.ts`) is the one that normally produces the plate/object index the web sees; it is a hand-kept mirror of the API's `apps/api/src/lib/three-mf-reader.ts` (the read/index half of the API's `three-mf-*` modules, used for the local/fallback parse; scene assembly and slice-time 3MF rewriting live in the sibling `three-mf-scene-builder.ts`/`three-mf-output.ts`), so changes to the parsed index shape must land in both parsers with their parser-cache versions and the API derived-index cache version bumped together. When a bridge is replaced, the API can recover printer assignments by matching rediscovered serials from the connected bridge against tenant printers that are orphaned or still assigned to a disconnected bridge. For non-Docker installs, the same runtime can also be wrapped in a small Electron tray host that surfaces bridge status, bridge ID, and pairing code from the local desktop. Hosted deployments can run a dedicated demo bridge entrypoint for the reserved public-demo tenant; the tracked cloud compose template wires that through a one-shot demo bootstrap job plus a dedicated demo bridge state file and library directory inside the shared app data volume. The production bridge uses a stable runner image plus signed self-contained app-bundle updates, documented in [docs/bridge-update-system.md](docs/bridge-update-system.md), and also reports build revision/source-fingerprint metadata so Settings can flag same-version bridge image drift. The public bridge runtime image excludes the API and web application directories.
...
| `apps/bridge` | Outbound LAN runtime for SSDP discovery plus bridged MQTT/FTPS/camera transport, optionally wrapped in a desktop tray host for packaged installs. |
...
| `/api/beta-signup` | Public marketing beta signup form delivery through the configured Cloudflare Email Sending transport |
| `/api/overview` | Tenant overview readiness and persisted workspace production stats |
| `/api/platform/overview` | Platform overview rollup stats, defaulting to all tenants with an optional own-workspaces scope |
| `/api/tenants` | Platform-level tenant directory CRUD with server-side filtering, sorting, and pagination |
...
| Page | Description |
|---|---|
| Marketing home | Public root homepage with product screenshots, beta status messaging, demo/login links, and a Cloudflare Email Sending-backed beta signup form. |
| Terms, privacy, and contact | Public document pages for service terms, privacy policy, and contact details, linked consistently from the public footer. |
| Platform overview | Tenantless landing page for the platform workspace, with all-tenant or own-workspace production stats plus quick links into tenant management and platform settings. |
| Platform tenants | Dedicated tenant directory route with server-backed search, sorting, and pagination. |
...
- Auth is plugin-provided and installation-controlled. The built-in `auth-local` and `auth-oauth` providers ship disabled on fresh installs until the operator enables and configures them.
