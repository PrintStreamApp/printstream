---
applyTo: "apps/api/src/lib/ws*.ts,apps/api/src/lib/*event*.ts,apps/api/src/lib/auth-context.ts,apps/api/src/routes/**,apps/api/src/plugins/**,apps/web/src/hooks/usePrinterWebSocket.ts,apps/web/src/lib/ws*.ts,apps/web/src/lib/apiClient.ts,apps/web/src/lib/workspaceContext.ts,apps/web/src/lib/*QueryInvalidation.ts,packages/shared/src/ws-events.ts"
description: "Automatically load the repo's data and event delivery contract when working on HTTP loading, WebSocket delivery, workspace context, cache invalidation, or auth-change events."
---

# Data And Event Delivery Instructions
