...
argument-hint: "Optional message hint or summary"
agent: "agent"
model: "GPT-5 (copilot)"
---
...
- Do not ask for confirmation before committing unless the prompt invocation explicitly requests a confirmation step.
- Treat an explicit `push` request in the prompt invocation as approval to run `git push` after a successful commit. Otherwise ask before pushing.
